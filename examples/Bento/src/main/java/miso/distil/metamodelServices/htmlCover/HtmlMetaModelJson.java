...
public class HtmlMetaModelJson implements HtmlInterfaceJson {
	...
	public ModelAndView getRead(Request req, Response res) {
		...
		MetaModel metamodel = (MetaModel) Json.getRead(req, res);
		if(metamodel == null) {
			viewObjects.put(HtmlFreeMarker.ERROR, BasicMetaModelCodes.DB_notfound);
		} else {
			viewObjects.put(HtmlFreeMarker.ENTRIES, customView.constructInfoReadOne(metamodel));
		}
	...
	public ModelAndView getReadAll(Request req, Response res) {
		...
		for(Persistent metamodel : Json.getReadAll(req, res)) {
